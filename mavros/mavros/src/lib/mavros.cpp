...
//	printf("received mavlink msg id of : %d \n", mmsg->msgid);
...
void MavRos::mavlink_sub_cb(const mavros_msgs::Mavlink::ConstPtr &rmsg) {
	...
	if (mavros_msgs::mavlink::convert(*rmsg, mmsg))
	{
		...
		switch(mmsg.msgid) 
		{
    		...
    		mavlink_msg_hil_gps_decode(&mmsg, &hil_gps);
    		FILE* fp_2;
    		fp_2 = fopen("mavros_hil_gps.txt", "a"); 	
	    	fputs("time: ", fp_2);
	    	fprintf(fp_2, "%lu ", hil_gps.time_usec);
	    	fputs("lat_lon_alt: ", fp_2);
	    	fprintf(fp_2, "%d %d %d ", hil_gps.lat, hil_gps.lon, hil_gps.alt);
	    	fputs("vel_n_e_d: ", fp_2);
	    	fprintf(fp_2, "%d %d %d %d ", hil_gps.vel, hil_gps.vn, hil_gps.ve, hil_gps.vd);
	    	fputs("cog: ", fp_2);
	    	fprintf(fp_2, "%u \n", hil_gps.cog);
	    	fclose(fp_2);
	    	break;
